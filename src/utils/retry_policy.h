...
public:
    ...
    virtual int32_t GetRetryInterval(int32_t retry_count) = 0;
};
...
public:
    ...
    int32_t GetRetryInterval(int32_t retry_count) override {
        return std::min(max_interval_ms_, base_interval_ms_ * (1 << std::min(retry_count, 31)));
    }
